 ...
 * - RETRY_MAX_ATTEMPTS (optional, default 4)
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (!retryable || !retriesLeft || timeLeftMs <= 0) {
        if (alerts.length === 0) {
          process.stderr.write(`hormuz_watch: websocket ended after ${attempt} attempt(s): ${String(err?.message ?? err)}\n`);
        }
        break;
      }
...
function parseConfig() {
  ...
  return {
    ...
    retryMaxAttempts: i('RETRY_MAX_ATTEMPTS', 4),
