...
test('sentinel_news chat tool (#86): present in both CHAT_TOOLS and botToolDefs (read-only), view-defaults the instrument, never guesses an unconfigured one', async () => {
  ...
  try {
    ...
    // #114: the tool injects settings-derived NEWSAPI_AI_* into the subprocess env,
    // so the spawned CLI resolves the provider from settings (not the bare env).
    const withKey = JSON.parse(execChatTool('sentinel_news', { instrument: 'WTICO/USD' }, { view: { instrument: 'XAU/USD', granularity: 'M5' }, settings: { NEWSAPI_AI_KEY: 'from-settings', NEWSAPI_AI_MODE: 'shadow' } }));
    assert.equal(withKey.meta.newsApiAiMode, 'shadow', 'settings NEWSAPI_AI_MODE reaches the spawned tool');
...
test('bot modal ships setup + strategy tabs (#75 structural)', async () => {
  await withServer(mkdtempSync(join(tmpdir(), 'ss-')), async ({ base }) => {
    ...
    // the page — check for the id-composing calls.
...
test('gap backfill: a hole in the stored window is repaired via a deferred ranged fetch (from=gap start), not blocking the GET', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'ss-'));
  const dbPath = join(dir, 'db.sqlite');
  const before = series([100, 101, 102], Date.parse('2026-07-22T08:00:00Z'));
  const after = series([110, 111, 112], Date.parse('2026-07-22T09:00:00Z')); // 55 min gap after `before` (>> 3x M5)
  ...
  await chartData(dbPath, INSTRUMENT, { fetcher });
  await new Promise((r) => setTimeout(r, 20));
  assert.equal(requests.length, 1, 'exactly one ranged repair fetch scheduled for the one gap');
  ...
  assert.ok(Number(repaired.n) > 0, 'the gap now has rows persisted');

  // A second chartData call for the same combo must NOT re-fetch the same gap
  // (still visible in the stored window until the repair rows fill it in, but
  // the attempted-memory prevents a second network hit regardless).
  await chartData(dbPath, INSTRUMENT, { fetcher });
  await new Promise((r) => setTimeout(r, 20));
